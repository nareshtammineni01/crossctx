...
let activeConfThreshold = 0;      // 0–1
let activeServiceFilter = new Set(); // empty = show all
...
function applyToolbarFilters() {
  ...
  // Hide service nodes with no visible edges if a filter is active
...
(function buildServiceFilterChips() {
  ...
  services.forEach((svc, i) => {
    ...
    chip.addEventListener('click', () => {
      ...
      // Rebuild filter set from active chips
      activeServiceFilter = new Set(
        Array.from(document.querySelectorAll('.svc-filter-chip.active'))
             .map(c => c.dataset.svcId)
      );
      // If all active or all inactive, treat as "show all"
