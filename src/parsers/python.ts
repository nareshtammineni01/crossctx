...
function isDTOFile(filePath: string, content: string): boolean {
  const fileName = path.basename(filePath, ".py").toLowerCase();
  return (
    ...
    content.includes("Serializer") ||
    content.includes("dataclass")
  );
}

function extractPythonPayloadShapes(content: string, framework: SupportedFramework): PayloadShape[] {
