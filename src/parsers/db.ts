...
export function extractDbUsage(fileContents: Map<string, string>, language: DbLanguage): DbUsage[] {
...
function extractTsDbUsage(content: string, filePath: string): DbUsage[] {
  ...
  lines.forEach((line, idx) => {
    ...
    const sequelizeDefineMatch = line.match(
      /(?:sequelize\.define|\.define)\s*\(\s*["']([^"']+)["']/,
    );
    ...
    if (sequelizeTableNameMatch) {
      usage.push({
        name: sequelizeTableNameMatch[1],
        dbType: "sql",
        accessPattern: "orm-model",
        sourceFile: filePath,
        line: lineNum,
    ...
    const redisMatch = line.match(
      /(?:redis|client|redisClient)\.(?:set|get|hset|hget|lpush|rpush)\s*\(\s*["'`]([^"'`\n:]+):/,
    );
    if (redisMatch) {
      usage.push({
        name: redisMatch[1],
        dbType: "redis",
        accessPattern: "cache-key",
        sourceFile: filePath,
        line: lineNum,
      });
    }
...
function extractPyDbUsage(content: string, filePath: string): DbUsage[] {
  ...
  lines.forEach((line, idx) => {
    ...
    const mongoAttrMatch = line.match(
      /(?:db|database)\.([a-z_]\w+)\s*\.(find|insert|update|delete|aggregate)/,
    );
    if (mongoAttrMatch) {
      usage.push({
        name: mongoAttrMatch[1],
        dbType: "mongodb",
        accessPattern: "collection",
        sourceFile: filePath,
        line: lineNum,
    ...
    const redisMatch = line.match(
      /(?:r|redis|redis_client|client)\.(set|get|hset|hget|lpush|sadd)\s*\(\s*["'f]?["']?([^"':{}\n]+):/,
    );
    if (redisMatch) {
      usage.push({
        name: redisMatch[2].trim(),
        dbType: "redis",
        accessPattern: "cache-key",
        sourceFile: filePath,
        line: lineNum,
...
function extractGoDbUsage(content: string, filePath: string): DbUsage[] {
  ...
  lines.forEach((line, idx) => {
    ...
    const redisMatch = line.match(
      /(?:rdb|redisClient|client)\.(?:Set|Get|HSet|HGet|LPush|SAdd)\s*\(\s*ctx\s*,\s*["']([^"':{}\n]+):/,
    );
    if (redisMatch) {
      usage.push({
        name: redisMatch[1].trim(),
        dbType: "redis",
        accessPattern: "cache-key",
        sourceFile: filePath,
        line: lineNum,
